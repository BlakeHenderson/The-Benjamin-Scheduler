...
def retrieveFirstNameData():
    ...
    cursor.execute ("SELECT `firstName` FROM `textMemberInfo` WHERE `sendTime`=700")
...
def retrieveLastNameData():
    ...
    connection = MySQLdb.connect (host = "localhost", user = "root", passwd = "", db = "")
    cursor = connection.cursor ()
    cursor.execute ("SELECT `lastName` FROM `textMemberInfo` WHERE `sendTime`=700")
...
def retrievePhoneData():
    ...
    connection = MySQLdb.connect (host = "localhost", user = "root", passwd = "", db = "")
    cursor = connection.cursor ()
    cursor.execute ("SELECT `phoneNumber` FROM `textMemberInfo` WHERE `sendTime`=700")
...
def retrieveCarrierData():
    ...
    connection = MySQLdb.connect (host = "localhost", user = "root", passwd = "", db = "")
    cursor = connection.cursor ()
    cursor.execute ("SELECT `carrier` FROM `textMemberInfo` WHERE `sendTime`=700")
